...
def get_projects_for_plant(plant_key, selected_date):
    """
    Fetch active projects for the given plant on selected_date.
    selected_date is a date object.
    """
    cfg = PLANTS[plant_key]
    conn = make_conn(cfg["server"], cfg["db"], cfg["user"], cfg["pwd"])
    cur = conn.cursor()
    # Use the selected date instead of always GETDATE()
    date_str = selected_date.strftime("%m/%d/%Y")  # matches CONVERT(varchar, ..., 110) format
    cur.execute("""
        SELECT a.TabId, a.ProjCode, b.DbName
        FROM TabProjDesc a
        INNER JOIN TabUtilityMaster b ON a.UtilityId = b.TabId
        WHERE a.LastAccessed >= ?
...
def get_hourly_for_project(server, proj_db, proj_id, proj_code, sa_pwd, selected_date):
    """
    Fetch hourly production data for a project on selected_date.
    selected_date is a date object.
    """
...
def index():
    ...
    return render_template(
        ...
        today=today_display,
        selected_date=selected_date.isoformat(),   # YYYY-MM-DD for the date input
    )
